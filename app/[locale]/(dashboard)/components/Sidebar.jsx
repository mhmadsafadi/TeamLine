      ...
      icon: (
        ...
        >
          <path
            ...
            strokeLinejoin="round"
            strokeWidth={2}
            d="M9.529 9.988a2.502 2.502 0 1 1 5 .191A2.441 2.441 0 0 1 12 12.5h-.5m0 3h.01M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"
          />
        </svg>
      ),
    },
            ...
            {navItems.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  onClick={onClose}
                  className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 group ${isActive(item.href) ? "bg-gradient-to-r from-main to-secondary text-white shadow-sm shadow-main/20" : "text-gray-600 hover:bg-gray-100 hover:text-main"}`}
                >
                  <span
                    className={
                      isActive(item.href)
