  ...
  return (
    <div className="flex flex-col bg-white rounded-2xl p-5 border border-gray-200 hover:border-main/30 hover:shadow-md transition-all duration-200">
      ...
      <div className="flex justify-between items-start gap-3 mb-3">
        ...
        <span
          className={`text-xs px-2.5 py-1 rounded-full border whitespace-nowrap font-medium ${statusColors[status]}`}
        >
          {t(`status.${status}`)}
        </span>
      </div>
            ...
            <>
              {visibleMembers.map((member, i) =>
                member.avatar_url ? (
                  ...
                  <div
                    key={i}
                    className="w-7 h-7 rounded-full border-2 border-white bg-gradient-to-r from-main to-secondary flex items-center justify-center text-white text-xs font-bold"
                  >
              ...
              {extraMembers > 0 && (
                <div className="w-7 h-7 rounded-full border-2 border-white bg-gray-100 flex items-center justify-center text-xs text-gray-500 font-medium">
                  +{extraMembers}
                </div>
              )}
            </>
